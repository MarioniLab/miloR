...
Let's check out the markers for group 10 for example

```{r}
gr10_markers <- nhood_markers[c("logFC_10", "adj.P.Val_10")] 
colnames(gr10_markers) <- c("logFC", "adj.P.Val")

head(gr10_markers[order(gr10_markers$adj.P.Val), ])
```
...
```{r}
nhood_markers <- findNhoodGroupMarkers(embryo_milo, da_results, subset.row = hvgs,
                                       subset.nhoods = da_results$NhoodGroup %in% c('11','10'),
                                       aggregate.samples = TRUE, sample_col = "sample")
...
```{r}
ggplot(nhood_markers, aes(logFC_10-log10(adj.P.Val_10 ))) + 
  geom_point(alpha=0.5, size=0.5) +
  geom_hline(yintercept = 2)

markers <- rownames(nhood_markers)[nhood_markers$adj.P.Val_10 < 0.01 & nhood_markers$logFC_10 > 0]
```
...
plotNhoodExpressionGroups(embryo_milo, da_results, features=markers,
                          subset.nhoods = da_results$NhoodGroup %in% c('11','10'), scale=TRUE,
                          grid.space = "fixed")
```
