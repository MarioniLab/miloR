...
colData(pbmc.milo)$ObsID <- paste(colData(pbmc.milo)$sampleid, colData(pbmc.milo)$tenx_lane)
...
da_results <- testNhoods(pbmc.milo, design = ~ adjmfc.time + (1|tenx_lane), design.df = pbmc.design, fdr.weighting="graph-overlap",
                         glmm.solver="HE-NNLS", REML=TRUE, norm.method="TMM", BPPARAM = bpparam)
...
can cause problems as there isn't much information/variance from which to learn the (locally) optimal parameter values.
...
plotUMAP(pbmc.milo, colour_by="adjmfc.time") + plotNhoodGraphDA(pbmc.milo, da_results, alpha=0.1) +
...
case for this is where you have multiple observations per experimental individual/sample and thus the nhood counts are not i.i.d. The most obvious case, as discussed in the intro 
