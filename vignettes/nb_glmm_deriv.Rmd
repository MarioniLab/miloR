...
From this we know that $t(y)=y$, $\theta=log(p)$ and $b(\theta)=-rlog(1-p)$ (and $a(\phi)=1???$. Therefore $p=e^{\theta}$. Using this formulation we can determine the mean and variance, because $\frac{d b(\theta)}{d \theta} = E[Y] = \mu$ and $Var[Y]=\phi \frac{d^2b(\theta)}{d\theta^2}$.
...
### Proof for variance of NB distribution

To determine the value of $\phi$, we need to find an expression for the variance of the negative binomial distribution. We start with the probability mass function of the negative binomial, as above. 

$$ P(X = y) = {y+r-1\choose y} (1-p)^{r}(p)^y $$
We know the formula for the expected value E(X). In our case:

$$E(X) = \sum_{y=0}^{\infty} yP(X=y) = \sum_{y=0}^{\infty}y{y+r-1\choose y} (1-p)^{r}(p)^y$$
We first consider the simple case where r = 1, as follows: 
...
To build the NB-GLMM, we require the variance term expressed in terms of $\mu$. First, rearrange to get an expression for p in terms of $\mu$:
...
(4) Link function: $\eta = g(\mu|b)$, or alternatively, inverse link function: $h(\eta) = X\beta + Zb$\\
...
We define the first order derivative of the inverse link function $h(\eta)$ evaluated at $\hat{\eta}$ as:

$$\frac{\partial h(\hat\eta)}{\partial\hat\eta} = h'(\hat{\eta}) \\
\hat{D} = diag[h'(\hat{\eta})] = diag[e^{\hat{\eta}}]$$
...
We implement the Fisher scoring algorithm to iteratively update our variance-covariance estimate:
...
Estimation of the GLMM proceeds iteratively: the process starts by initial estimates of $\beta$, b and $\sigma$. These are used in the GLMM estimating equations to compute updated solutions for $\beta$ and b. Next, the currect estimates of $\beta$ and b are used to calculate values for the score and information matrix, and to update $\sigma$ with the Fisher scoring algorithm. Estimation should continue until convergence.