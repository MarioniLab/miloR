context("Testing calcNeighbourhoodExpression function")
library(miloR)
...
test_that("Incorrect/missing inputs produce proper errors", {
    # test without running makeNeighbourhoods
    expect_error(calcNeighbourhoodExpression(sim1.mylo),
                 "No neighbourhoods found - run makeNeighbourhoods first")

    # pass a list but no expression
    sim1.mylo <- makeNeighbourhoods(sim1.mylo, k=21, prop=0.1, refined=TRUE,
                                    d=30,
                                    reduced_dims="PCA")
    expect_error(calcNeighbourhoodExpression(neighbourhoods(sim1.mylo)),
                 "No expression data found. Please specific a gene expression matrix to exprs")
})
...
test_that("Same output is generated by different input types", {
    # do the Milo and list inputs generate the same result?
    expect_equal(neighbourhoodExpression(calcNeighbourhoodExpression(neighbourhoods(sim1.mylo),
                                                                     exprs=logcounts(sim1.mylo))),
                 neighbourhoodExpression(calcNeighbourhoodExpression(sim1.mylo)))
})

test_that("Subsetting produces the expected output dimensionality", {
    set.seed(42)
    subset.genes <- sample(1:nrow(sim1.mylo), size=500)

    # test with different inputs
    expect_identical(nrow(neighbourhoodExpression(calcNeighbourhoodExpression(sim1.mylo, subset.row=subset.genes))),
                     length(subset.genes))

    expect_identical(nrow(neighbourhoodExpression(calcNeighbourhoodExpression(neighbourhoods(sim1.mylo),
                                                                               exprs=logcounts(sim1.mylo),
                                                                               subset.row=subset.genes))),
                     length(subset.genes))
})
