...
set.seed(42)
random.levels <- list("Fam"=unique(as.numeric(as.factor(sim.df$Fam))))
...
test_that("Discordant input matrices give errors", {
    ...
    expect_error(fitGLMM(X=X, Z=Z, y=y[seq_len(nrow(X)-1)], offsets=rep(0, nrow(X)), random.levels=random.levels, REML = TRUE,
                         dispersion=dispersion), "Dimensions of")
    ...
    expect_error(fitGLMM(X=X, Z=Z, y=y, offsets=rep(0, nrow(X)), Kin=kin[seq_len(nrow(kin)-1), , drop=FALSE],
                         random.levels=random.levels, REML = TRUE, dispersion=dispersion), "Input covariance matrix is not square")
    ...
    # non-square Z in covariance only model
    expect_error(fitGLMM(X=X[seq_len(nrow(g.Z)-1), , drop=FALSE], Z=g.Z[seq_len(nrow(g.Z)-1), , drop=FALSE],
                         y=y[seq_len(nrow(g.Z)-1)], offsets=rep(0, nrow(X)), Kin=kin, geno.only=TRUE,
                         random.levels=random.levels, REML = TRUE, dispersion=dispersion), "Z matrix is not square")
    ...
    expect_error(fitGLMM(X=X, Z=wrong.Z,
                         ...
                         random.levels=random.levels, REML = TRUE, dispersion=dispersion),
    ...
    expect_error(fitGLMM(X=X, Z=inv.Z, y=y, offsets=rep(0, nrow(X)), random.levels=random.levels, REML = TRUE,
                         dispersion=dispersion), "Columns of Z must have valid names")
    ...
    expect_error(fitGLMM(X=fail.X, Z=fail.Z, y=fail.y, offsets=rep(0, nrow(X)), random.levels=fail.random.levels, REML = TRUE,
                         dispersion=dispersion), "Columns of Z are discordant with input random effect levels")
...
test_that("Infinite and NA values fail as expected", {
    ...
    expect_error(fitGLMM(X=X, Z=Z, y=y, offsets=inf.offsets, random.levels=random.levels, REML = TRUE,
                         dispersion=dispersion), "Infinite values in initial estimates")

    na.offsets <- rep(0, nrow(X))
    na.offsets[sample(length(na.offsets), size=1)] <- NA
    expect_warning(fitGLMM(X=X, Z=Z, y=y, offsets=na.offsets, random.levels=random.levels, REML = TRUE,
                         dispersion=dispersion), "NA values in offsets")

    na.X <- X
    na.X[sample(seq_len(nrow(X)), size=1), 2] <- NA
    expect_warning(fitGLMM(X=na.X, Z=Z, y=y, offsets=rep(0, nrow(X)), random.levels=random.levels, REML = TRUE,
                           dispersion=dispersion), "(NAs values in initial estimates)|(NA parameter estimates)")
    ...
    expect_error(fitGLMM(X=X, Z=Z, y=y, offsets=rep(10000, nrow(X)), random.levels=random.levels, REML = TRUE,
                         dispersion=dispersion), "Infinite values in initial estimates - reconsider model")
...
test_that("Forcing a singular Hessian gives errors", {
    ...
    random.levels <- list("RE1"=unique(as.numeric(as.factor(sim.df$FE1))))
    ...
    expect_error(fitGLMM(X=X, Z=Z, y=y, offsets=rep(0, nrow(X)), random.levels=random.levels, REML = TRUE,
                         dispersion=dispersion), "Hessian is computationally singular")
